...
> 当前版本：`v1.5.1` · 插画化三栏阅读、编辑、AI 辅助、Notebook 与正文标记
>
...
## v1.5.1 本轮完成：矢量化视觉与平滑折叠修正

- 中间栏旋钮升级为独立于文件内容的持久 dock；切换文件时复用整个控件节点，避免闪烁和卡顿。
- 旋钮左侧对应只读、右侧对应编辑；点击和左右方向键均可操作，并在 reduced-motion 下静态切换。
- 接入初始阅读插画、AI 空状态蝴蝶、文件栏插画标题；新增插画均为透明背景 SVG 路径，不嵌入 PNG、滤镜或悬浮阴影。
- 折叠图标按 Hana 现有面板几何关系改为内联 SVG；展开入口固定在窄条顶部。
- 侧栏折叠窄条加入下半部错落海浪动效，SVG 以错落尺寸从侧边涌入；工作区 grid 使用平滑过渡，并提供 reduced-motion 降级。
...
当前本地目标：`feat/reader-visual-simplification` / `v1.5.1`；稳定基线：`main` / `v0.9.0`。本轮只做本地提交，不自动推送或合并。
