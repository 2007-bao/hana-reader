...
test('manifest declares the v1.3.4 reader page with guarded resource access', async () => {
...
test('reader source, built assets, and cache-busting route are present', async () => {
  ...
  assert.match(route, /ASSET_REVISION = '1\.3\.4'/);
  ...
  assert.match(panelSource, /const PLUGIN_VERSION = '1.3.4'/);
...
test('visual simplification keeps annotations, notebook, and Ctrl-Z paths local', async () => {
  ...
  assert.match(css, /--reader-side-bg: #eceeef/);
