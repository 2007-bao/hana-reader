...
当前 v2.0.0 已将 Maple Mono 字体、语义色彩、内容排版和代码 token 作为阅界视觉系统的一部分，并保留 `assets/fonts/OFL.txt` 与必要的来源说明。早期版本中关于 v0.6.1、v0.7.0 的过程记录只用于追溯，不代表当前实现状态。

后续若继续调整 Maple 相关内容，应优先复用上游变量与资源，先做小范围视觉回归，再修改阅界自身的适配层。
